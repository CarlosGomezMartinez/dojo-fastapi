...
def get_book_by_isbn(isbn: str, response: Response):
    ...
    if not book:
        ...
        return {"message":"Libro no encontrado" , "status_code":404}
...
def create_book(book: BookEntity, response: Response):
    ...
    if not book_db:
        ...
        return {"message": "El libro fue creado", "status_code": 201, "body": book_db}
...
def update_book(isbn: str ,book: BookEntityUpdate, response: Response):
    ...
    if not book_db:
        response.status_code = status.HTTP_404_NOT_FOUND
        return {"message": "El elemento que busca no existe", "status_code": 404}
    book_db = book_repository.update_book(isbn,book)
    response.status_code = status.HTTP_200_OK
    return {"message": "Libro actualizado", "status_code": 200, "body": book_db}
...
def delete_book(isbn: str, response: Response):
    ...
    if not book_db:
        ...
        return {"message": "El elemento que busca no existe", "status_code": 404}
    response.status_code = status.HTTP_200_OK
    return {"message": "Libro eliminado", "status_code": 200, "body": book_db}